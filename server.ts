...
async function createServer() {
  ...
  app.get('/api/resume/history', authenticateToken, async (req: AuthenticatedRequest, res) => {
    ...
    if (useFirebase) {
      try {
        const dbAdmin = getFirestore();
        const snapshot = await dbAdmin.collection('resumes').where('userId', '==', req.user.id).get();
  ...
  app.delete('/api/resume/:id', authenticateToken, async (req: AuthenticatedRequest, res) => {
    ...
    if (useFirebase) {
      try {
        const dbAdmin = getFirestore();
        const docRef = dbAdmin.collection('resumes').doc(resumeId);
        const docSnap = await docRef.get();
        if (!docSnap.exists || docSnap.data()?.userId !== req.user.id) {
  ...
  app.post('/api/resume/analyze', authenticateToken, async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      console.log('[resume/analyze] Sending request to Gemini API...');
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Gemini API request timed out after 60 seconds. Check your GEMINI_API_KEY and network/firewall settings.')), 60000);
      });
